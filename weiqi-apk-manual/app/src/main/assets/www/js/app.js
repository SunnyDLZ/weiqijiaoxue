...
class WeiqiApp {
  ...
  initPvPBoard() {
    ...
    document.getElementById('btnPassPvP').onclick = () => {
      sound.playClick();
      if (this.engine.gameOver) return;
      this.engine.pass();
  ...
  showAIDifficultySelect() {
    ...
    container.innerHTML = `
      ...
      <div class="ai-start-row">
        ...
        <button class="btn btn-success" id="btnStartAI">开始对局</button>
      </div>
  ...
  updateAIGameInfo() {
    ...
    document.getElementById('ai-diff-badge').textContent =
      '难度：' + ({ normal: '一般', hard: '困难', nightmare: '噩梦' })[this.aiDifficulty];
    ...
    container.innerHTML = `
      ...
      <div id="gomoku-ai-diff-select" style="display:none; margin-top: 16px;">
        ...
        <div class="ai-start-row">
          ...
          <button class="btn btn-success" id="btnStartGomokuAI">开始对局</button>
        </div>
      </div>
    `;
  ...
  async gomokuAIMove() {
    ...
    document.getElementById('gomoku-ai-thinking').classList.add('active');
    ...
    try {
      ...
      document.getElementById('gomoku-ai-thinking').classList.remove('active');
      this.isGomokuAiThinking = false;
    }
  }
  ...
  updateGomokuAIInfo() {
    ...
    document.getElementById('gomoku-ai-diff-badge').textContent =
      '难度：' + ({ easy: '简单', medium: '中等', hard: '困难' })[this.gomokuDifficulty];
  }
