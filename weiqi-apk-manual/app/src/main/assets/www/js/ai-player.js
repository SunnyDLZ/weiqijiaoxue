...
class AIPlayer {
  constructor(engine, difficulty = 'normal') {
    ...
    this.simulations = { normal: 10, hard: 60, nightmare: 120 };
    this.thinking = false;
  }
  ...
  async getNormalMoveAsync(legalMoves) {
    ...
    const scored = [];
    const batchSize = Math.min(30, Math.ceil(legalMoves.length / 5));
    ...
    for (let i = 0; i < legalMoves.length; i++) {
      ...
      if (result.success) {
        ...
        const sampleSize = Math.min(oppMoves.length, 15);
        let danger = 0;
  ...
  async getHardMoveAsync(legalMoves) {
    ...
    const scored = [];
    const opponent = aiColor === 1 ? 2 : 1;
    const batchSize = Math.min(20, Math.ceil(legalMoves.length / 5));
    ...
    for (let i = 0; i < legalMoves.length; i++) {
      ...
      if (oppMoves.length > 0) {
        ...
        const sampleCount = Math.min(oppMoves.length, 12);
  ...
  async getNightmareMove(legalMoves) {
    ...
    // 限制候选移动数量（10个，避免搜索量爆炸）
    const candidates = this.getCandidateMoves(legalMoves, 10);
